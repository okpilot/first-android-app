...
| **Refinement of ↑:** reversal sweep misses SECONDARY stale surfaces *within an already-touched file* — summary/conventions blocks + decision-entry SUBSECTIONS (Context/Impl/Principle), not the obvious rule line. First: `3296258`. | 2 | d549d45 | **RULE CANDIDATE** — recurred `d549d45`: plan sweep missed D33 (decisions.md L390-391) + plan.md L41/L67-68, proposed skipping migration headers Slices 2/3 fixed in-slice; plan-critic caught it (REVISE). Refinement lives only in doc-updater POST-COMMIT memory; binding `CLAUDE.md` rule says only "grep every sibling **file**". Propose ONE clause into that paragraph: "grep the WHOLE of each touched file + every ledger subsection, not just the first citation". NOT a `/fullpush` grep gate (semantic, un-greppable per-slice → double-gates plan-critic+doc-updater). |
...
| **Doc surface states `auth.uid()` as PRESENT-TENSE requirement while YAML/rule #2 phase-defer it** — `docs/database.md` #6. First: `d549d45` (coderabbit-sync ISSUE, fixed in-cycle: #6 now "once auth is wired (#3) — deferred until GoTrue lands"). | 1 | d549d45 | WATCHING — phase-caveat on a NEW doc surface. Recurs on another surface → propose a phase-aware clarification pass, NOT enforcement (see false-positive traps). |
...
- **Rule-reversal-sync (PROMOTED, count 3, `CLAUDE.md`) + its refinement (RULE CANDIDATE, count 2).**
  The binding rule catches sibling FILES; the recurring miss is SECONDARY surfaces WITHIN a touched
  file + ledger subsections — a refinement currently living only in doc-updater's post-commit memory.
  Proposed fix folds it up into the CLAUDE.md paragraph (plan-time). Deliberately NOT a mechanical
  `/fullpush` grep (semantic, per-slice phrasing; plan-critic+doc-updater already gate it). Every
  recurrence caught in-cycle — the clause reduces re-derivation, does not plug a leak.

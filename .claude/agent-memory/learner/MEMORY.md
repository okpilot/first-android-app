...
| **Adding a scalar field to a model silently DROPS it in hand-fake repos that RECONSTRUCT the entity** (`_StatefulTasksRepo.create/archive/restore` rebuild `Task(...)` from scratch, not pass-through) AND breaks exact-map `toRpcParams()` assertions. Invisible to analyze/lint/hooks/CR (test-fake completeness). First: `notes` (task Slice 1). | 3 | 3bf48ea | **PROMOTED → CLAUDE.md "How we work"** ("Adding a field to a model isn't done until every hand-fake reflects it", written 3bf48ea follow-up). Recurred notes→contacts→`importance` = 3 distinct commits. Split OUT of meta-cluster sub-mechanism (d). |
...
- **Meta-cluster: "under-scoped sibling-surface sweep on a change" (do NOT promote — mixed
  ...
  learner-PROPOSED → CLAUDE.md** (no longer part of this cluster's count); (e) ADD `p_*` param→sibling INLINE

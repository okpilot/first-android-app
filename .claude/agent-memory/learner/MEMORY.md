...
| **Width/breakpoint-parameterized widget tests need a deterministic surface-size lever + teardown.** A test that asserts responsive layout (sidebar vs bottom-nav at a breakpoint) must set `tester.view.physicalSize` **and** `tester.view.devicePixelRatio` (logical width = physical/DPR — setting physicalSize alone is wrong if DPR≠1), then `addTearDown(tester.view.reset…)` so the fake size doesn't leak into sibling tests. First (only) sighting: desktop-sidebar Slice A `4679504` (plan-critic caught the physicalSize-vs-DPR lever gotcha at plan time, fixed before build). | 1 | 4679504 | WATCHING — first responsive/width-driven test in the project; single sighting → log & watch, NO rule. If a SECOND width/breakpoint test trips the same lever/teardown gotcha (leaked surface size, DPR-unaware width, or missing reset) → RULE CANDIDATE (propose ONE `docs/design-principles.md` or test-convention line). Not gated by analyze/lint. |

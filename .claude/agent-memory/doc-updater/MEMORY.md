...
- **Widget extraction / refactor for follow-on slices** (e.g. `CommentsSection` extract enabling task comments): update `plan.md` *Current status* + `HANDOVER.md` Status + RESUME to name the refactor slice (e.g. "Slice 2a") as on-branch, note it as behavior-preserving, and point RESUME at the follow-on slice (Slice 2b). No new Decision appended (the decision is the follow-on feature, not the refactor). Both docs together.

...
| Near-identical contact-roster/chips **widget** duplicated per parent entity (Slice: link contacts to tasks). `_PeopleSection` (task_form) vs `_AttendeesSection` (event_form) differ by only 2 string literals (label 'PEOPLE'/'ATTENDEES', button 'Add people'/'Add contacts') — otherwise byte-identical Wrap-of-InputChips. `_PeopleList` (task_detail) vs `_AttendeeList` (event_detail): per-contact roster Row (avatar+name+company) byte-identical; differ only in header noun, source type (List<Contact> vs Event), empty branch. Both landed as DOCUMENTED mirrors ("Mirrors the event form's `_AttendeesSection`"). Same shape as the `MetaLine` extraction (PROMOTED). | 2026-07-14 (link contacts to tasks) | 1 | 2026-07-14 (2b100b7) | WATCHING — reported as SUGGESTION (non-blocking); deliberate documented mirror at N=2 parents. If a 3rd parent gets a contact-roster/chips widget, count→2 → RULE CANDIDATE (extract `ContactChipsSection` + shared roster row, à la MetaLine). |

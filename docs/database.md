...
6. **SECURITY DEFINER functions** always `SET search_path = public`; client-facing ones check `auth.uid()` **once auth is wired (issue #3)** — deferred until GoTrue lands, along with the `SET search_path = ''` hardening (see rule #2's Decision 36 note).

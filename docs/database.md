...
2. **All writes via RPC; reads direct-under-RLS.** Every write (INSERT / UPDATE / soft-delete) goes through a `SECURITY DEFINER` RPC; ordinary reads go direct to the table/view under RLS. One write path per entity is the natural home for `auth.uid()` owner checks when auth (issue #3) lands. *(Updated 2026-07-12, **Decision 26** — a knowing re-reversal of the prior "RPC for what matters, not everything is RPC" wording: uniformity + the auth payoff won over case-by-case. **Migration in progress:** `events`, `contacts` and `event_types` write via RPC; `event_comments` converts in Slice 3 — until then it still writes direct, and rule #4's `event_comments` exception below still holds.)*

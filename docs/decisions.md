...
**Decided:**
- **`task_contacts` join table** — the twin of `event_attendees`: `task_id → tasks(id) on delete cascade`, `contact_id → contacts(id) on delete cascade`, composite PK, reverse index on `contact_id` (so "which tasks is this contact on?" doesn't scan). RLS with a `using (true)` SELECT policy (like `event_attendees`) so archived tasks' People still embed (the **parent-gate difference noted at 20260714160000_add_contacts_to_tasks.sql:36-42**, not a database.md #4 case: task_contacts has no deleted_at and isn't self-soft-deletable). No write policy/grant — membership set only by the task-write RPCs.

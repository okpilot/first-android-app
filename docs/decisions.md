...
**Decided — three auth-independent hardenings (all behavior-preserving; `events`/`event_attendees` already ran SELECT-only-for-anon + RPC-only-writes, proving the pattern):**
...
- **Still deferred to #3** (this is now the *whole* remaining scope of the issue): `auth.uid()` ownership checks + owner-based RLS (need GoTrue), and the `SET search_path = ''` + schema-qualify convention hardening (a separate ripple-heavy slice, kept out to keep this one behavior-preserving).

...
- **Removed the generator's 16% foreground `<inset>`** in `mipmap-anydpi-v26/ic_launcher.xml`: the `C⁺` already sits inside the 72dp adaptive safe circle (the `⁺` at ~158px vs the ~170px safe radius on a 512 grid), so the inset only shrank the mark below the chosen framing for no clip-safety gain. Without it, the adaptive icon matches the legacy tile. (Re-drop the inset if a future `flutter_launcher_icons` run re-adds it.)

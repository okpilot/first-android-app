...
- 🔨 **App identity — launcher `CRM+` + dark `C⁺` icon (Decision 24) — PR #22 OPEN, pending on-device QA + merge.** Renamed `android:label` → `CRM+`; generated all mipmap densities + a modern adaptive icon via `flutter_launcher_icons` from the user's dark `C⁺` mark. Reproducible sources (SVG + PNG) committed under `assets/icon/`; adaptive foreground is a clean transparent glyph on a `#0a0a0a` background (no card-outline artifact under the mask); inset driven by `adaptive_icon_foreground_inset: 0` in config. Android-only. Commits `0e312f9` (feat) + `1fff1ee` (cloud-CR fix). Gate green (analyze · 69 tests · web build); `/crlocal` 2 clean rounds; cloud-CR cycle 1 answered (1 finding FIX). **Not merged — `main` stays green until the C⁺ is eyeballed on the physical S23+ (`/updatephone`, light+dark).**
- ✅ Decisions made — **24** (see `docs/decisions.md`): + design-principles adoption, local backend, Android SDK, Contacts slice, bespoke theme, homebase deploy, git hooks, calendar shell, /replycoderabbit, calendar events, **event types (colour-as-data)**, **cloud-CR two-command split (D20)**, **README Features section over a docs site (D21)**, **full LMS-Plus agent fleet (D22)**, **event comments (viewable soft-delete, D23)**, **app identity — `CRM+` + dark `C⁺` icon (D24)**.

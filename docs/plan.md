...
- 🔨 **RPC-for-all-writes (Decision 26) — IN FLIGHT.** Slice 0 (PostgREST auto-reload via DDL-watch triggers) MERGED (`670787c`, PR #25); resume = deploy+verify on homebase, then remove the manual reload, then Slices 1–3 (contacts/event_types/comments writes → RPC). Root-caused from the event-comment 404 = PostgREST stale-schema-cache (Decision 25; deploy script now reloads).
...
## Next slice
**RPC-for-all-writes — Decision 26 (IN FLIGHT).** Standardize every write onto PostgREST RPCs; reads stay
direct. Plan: `~/.local/share/claude-config/claude/plans/stuck-lazy-sutton.md`.
- ✅ **Slice 0 — DDL-watch auto-reload triggers MERGED** (`670787c`, PR #25).
- ⏭️ **RESUME:** deploy Slice 0 to homebase → verify auto-reload live (create+drop a throwaway object,
  confirm PostgREST serves it with no manual `NOTIFY`) → follow-up PR removes the manual reload from
  `deploy-homebase.sh`. Then **Slice 1** contacts writes → RPC (also rewrites database.md rule #2 — the
  audited-rule reversal), **Slice 2** event_types, **Slice 3** comments (rewrites rule #4 exception).

...
- ✅ **App identity — launcher `CRM+` + dark `C⁺` icon (Decision 24).** Renamed `android:label` → `CRM+`; generated all mipmap densities + a modern adaptive icon via `flutter_launcher_icons` from the user's dark `C⁺` mark. Reproducible sources (SVG + PNG) committed under `assets/icon/`; adaptive foreground is a clean transparent glyph on a `#0a0a0a` background (no card-outline artifact under the mask). Android-only.

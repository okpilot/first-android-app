...
- 🔨 **Cloud-CR tooling split — PR #16 open (`chore/coderabbit-commands`).** Replaced the single `/replycoderabbit` with **`/coderabbit`** (triage) + **`/replycoderabbit`** (reply-only) + shared `scripts/cr-findings.sh` (36-assertion test). Designed via 3 critic rounds + hardened via 3 `/crlocal` rounds. Decision 20. **Awaiting cloud CodeRabbit review** (round-4 local was rate-limited — cloud is the authoritative check).
...
## Next slice
**Event types — DONE, merged & deployed (Slices 1–3, PRs #13 → #14 → #15).** No further
event-types work planned. **First: merge PR #16** (the `/coderabbit` split) once cloud CR + CI are
green — a natural first dogfood of the new triage→reply flow. Then pick the next thin slice below.

> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (RPC-for-all-writes: Slice 0 deployed & verified).

# Handover

**Status: RPC-FOR-ALL-WRITES (Decision 26) — IN FLIGHT. Slice 0 (auto-reload) DEPLOYED & VERIFIED; next = Slice 1 (contacts writes → RPC).**
...
- ⏭️ **RESUME HERE — Slice 1:** contacts writes → RPC; also **rewrites database.md rule #2** (Decision 26 knowingly re-reverses the "NOT everything is RPC" rule) + stale repo doc-comments. Then **Slice 2** event_types, **Slice 3** comments (rewrites rule #4 comments exception). Convention: match the event RPC template (`security definer`, `set search_path = public`, grant anon+authenticated, `p_`-params, server-side `nullif(trim())`); new models need `toRpcParams()`; `update_comment` is `(p_id,p_body)` only. events already fully RPC (template). `auth.uid()` deferred to #3.

> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (RPC-for-all-writes: Slice 1 PR #27 open, awaiting cloud-CR triage + merge).

# Handover

**Status: RPC-FOR-ALL-WRITES (Decision 26) — IN FLIGHT. Slice 0 (auto-reload) DEPLOYED & VERIFIED (merged); Slice 1 (contacts writes → RPC) in OPEN PR #27; next = Slice 2 (event_types writes → RPC).**
...
- 🔄 **Slice 1 — contacts writes → RPC (OPEN PR #27)** (`5294ce1`, `feat/contacts-write-rpcs`): migration `20260712130000_contact_write_rpcs.sql` adds `create_contact` / `update_contact` SECURITY DEFINER RPCs (server-side trim + nullif normalization, update guards deleted_at + raises no_data_found, grants to anon+authenticated); `Contact.toRpcParams()` replaces `toWrite()` (+`_emptyToNull` removed); `contacts_repository` routed to RPCs + `_fetchOne` refetch; `docs/database.md` rule #2 re-reversed (Decision 26 dated); `.coderabbit.yaml` SQL rule + a new CLAUDE.md rule-reversal-sync workflow line (learner, count 2); `backend/README.md` verify curls; `event_types_repository.dart` doc-comment patched. **71 tests green, all 12 migrations on fresh postgres:16, RPCs exercised as anon.** Fleet all clean (db-security PASS; 1 deferrable `revoke execute from public` → #3). CI build pass. **RESUME: cloud CodeRabbit was PENDING at wrap — run `/coderabbit` on #27 → merge → `deploy-homebase.sh` → phone-QA add/edit contact.**
- ⏭️ **NEXT — Slice 2:** event_types writes → RPC (`create_event_type` / `update_event_type`). Match the template: security definer, `set search_path = public`, grant anon+authenticated, `p_`-params, server-side `nullif(trim())`; new model `toRpcParams()`. **Apply the new CLAUDE.md rule:** grep sibling doc-comments/headers citing the old rule and fix in-slice (the `event_types_repository` doc-comment I patched already reads "Pending Slice 2"). Then **Slice 3:** event_comments writes → RPC (rewrites rule #4 exception + comments-repo doc-comments). `auth.uid()` deferred to #3.

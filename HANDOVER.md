> Cross-session work tracker. Update in place. Last updated: 2026-07-14 (Task People — link contacts to tasks, on branch; owed: homebase deploy + QA + 2 prior migrations).

# Handover

**Status: TASK PEOPLE — link contacts to tasks — BUILT ON BRANCH `feat/task-people` (commit 2b100b7), pending /fullpush/PR/merge/deploy.** New `task_contacts` join table (mirroring `event_attendees`); updated `create_task(p_title, p_notes, p_contacts)` and `update_task(p_id, p_title, p_is_done, p_notes, p_contacts)` via the drop+recreate pattern (grants re-issued). `Task` model + repo threaded for `contacts` list (id/name/company). Task form People picker section (`ContactPickerScreen` generalized from event version); task detail read-only roster. Hard-DELETE of join rows on update (annotated exception — membership is derived). Decision 34. Feature lives on `main` via PR #38 (task comments, merged `b608afb`), PR #37 (CommentsSection extract, merged `ec9276a`), and this new commit/PR. **RESUME = homebase deploy (this feature PLUS the owed Decision 31 notes + Decision 33 task_comments migrations together) + light/dark emulator + Linux QA of People + comments + notes all in one pass.** Then backlog: #21 (in-app empty-state hints) · #3 (auth/GoTrue + DB hardening).

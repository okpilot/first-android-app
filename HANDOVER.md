> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (TASKS v0 (Decision 27) MERGED (PR #30 → squash `fe939d1`) & DEPLOYED to homebase (ledger 15); only phone QA remains owed).

# Handover

**Status: TASKS (v0) — Decision 27 — ✅ MERGED (PR #30 → squash `fe939d1`) & DEPLOYED to homebase. Only phone QA remains owed.**
A lightweight to-do list: add (form + FAB) · complete (tap the circle) · edit · archive · restore · view Completed + Archived (collapsible sections). New `tasks` table (`id`, `title`, `is_done`, `created_at`, `updated_at`, `deleted_at`), migration `20260712160000_create_tasks.sql`; **viewable soft-delete** reusing Decision 23's `using (true)` SELECT policy (2nd such table alongside `event_comments`); **writes via RPC** (Decision 26) — `create_task` / `update_task` (title+done in one path) / `soft_delete_task` / `restore_task`, SECURITY DEFINER for uniformity. Client: `Task` model + `TasksRepository` + `TasksListScreen` + `TaskFormScreen` + a 4th nav destination (Contacts · Calendar · **Tasks** · Settings), wired through `main`/`app`/`home_shell`. Full plan: `~/.local/share/claude-config/claude/plans/smooth-splashing-avalanche.md`.
- Gate green (`flutter analyze` · **101 tests** · web build · fresh-DB migration · db-security DEFERRABLE · `/crlocal` 4 rounds). Full 8-agent fleet run — semantic-reviewer caught 1 real bug (archived form left write affordances live → misleading error; fixed read-only + test-locked); `learner` promoted the `toRpcParams`↔RPC-arity trap to a `database.md` #2 rule (count 2). Plan-critic REVISE folded in pre-approval (create_task arity → `toRpcParams()` is create-shape only; the 2nd `ContactsApp` caller in `calendar_screen_test.dart` fixed).
- **Cloud CR cycle 1 answered:** 2 findings → both **FIX** (`4827161` — added the `identical(future,_future)` stale-guard to `TasksListScreen._load` matching `event_types_screen`; `unawaited(_load())` in `_openForm`); replied; both threads resolved.
- **Deployed to homebase** via `deploy-homebase.sh` (ledger 14 → **15**); verified live: `GET /rest/v1/tasks` → `200 []`, blank-title `create_task` → 400 `tasks_title_check` (RPC registered, no row written, tx rolled back).
- **Debug APK (HEAD `390ed54`, built vs `dev-defines.homebase.json`) installed on the S23+** via `/updatephone`, but **QA NOT completed** — the phone was off the tailnet (airplane / Tailscale disconnected), so the app showed "Couldn't load tasks". A single screencap confirmed the chrome renders (Tasks tab + check-circle icon, error-state styling, mono FAB in dark) but that is NOT the QA pass.
- ⚠️ **Process miss (owned):** the light+dark **visual-fidelity QA** ([[feedback-visual-qa-and-critics]]) was **skipped, unstated,** and the slice was merged + deployed before it ran — the 2nd recurrence. Memory tightened: visual QA is a HARD pre-`/fullpush` gate; if deferred, flag it, never silent.
**RESUME = (1) connect the S23+ to Tailscale, relaunch the app, and do the owed light+dark visual + functional QA (add / complete / edit / archive / restore) against homebase; (2) the Slice 1–3 RPC phone QA is still owed too — batch them.** Only then is Tasks v0 fully "done".

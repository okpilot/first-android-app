> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (RPC-for-all-writes: Slice 2 (event_types) MERGED (PR #28 → `a17ea81`) & DEPLOYED to homebase; resume = Slice 3 (event_comments), phone QA still owed).

# Handover

**Status: RPC-FOR-ALL-WRITES (Decision 26) — IN FLIGHT. Slices 0 (auto-reload), 1 (contacts) & 2 (event_types) DEPLOYED & VERIFIED (merged); next = Slice 3 (event_comments writes → RPC). Phone QA of the RPC write paths still owed.**
...
- ⏭️ **NEXT — Slice 3:** event_comments writes → RPC (`create_comment(p_event_id,p_body)` / `update_comment(p_id,p_body)` **body-only** / `soft_delete_comment(p_id)` / `restore_comment(p_id)`). SECURITY DEFINER for **uniformity** (comments' `using(true)` SELECT has no 42501 to dodge — note in header so red-team/db-security don't flag it). Model `toRpcParams()`; `comments_repository` → `.rpc()`. **Docs (the reversal):** rewrite `database.md` rule #4 event_comments exception + the comments-repo doc-comments (they say "direct, no RPC needed") — first real test of the promoted CLAUDE.md rule-reversal-sync rule. `auth.uid()` deferred to #3.
- 📱 **Phone QA still owed** for Slices 1–2 (add/edit a contact + add/edit an event type against homebase via `/updatephone` → S23+) — deferred across the slices; do before/with the Slice 3 device QA.

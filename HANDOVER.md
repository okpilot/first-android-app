> Cross-session work tracker. Update in place. Last updated: 2026-07-17 (Slice A2 — Edit → top-right, Decision 49 — **🔨 BUILT on branch `feat/edit-to-appbar`; committed, `/fullpush` + QA owed before merge**). Prior: UI consistency pass, Decisions 47 + 48 — ✅ MERGED, PR #55 → squash `154271b`.

# Handover

**Status: Slice A2 — Edit → top-right (finding #2, Decision 49) — 🔨 BUILT on branch `feat/edit-to-appbar` (committed; NOT pushed/merged).** User chose **full consistency**: Edit reads top-right in both layouts — the phone **AppBar** AND a new shared **`PaneHeader`** strip on the AppBar-less desktop master-detail pane — as the app's existing **`SubtleButton('Edit')`** tonal chip (the Decision 29 affordance, NOT a bare pencil; the event detail's old pencil was switched to the same chip here). **Single-sourced, no state refactor:** the detail views keep owning `_edit()`; a `showPaneHeader` flag (default false → phone tree pixel-identical) makes the view draw its own strip, and the phone AppBar reaches the view's public `edit()` via a `GlobalKey` (safe — the narrow body view is never re-keyed). Both routes go through the guarded `edit()` (no-ops busy/deleting; archived → no Edit on the desktop strip too). Reverses Decision 29's "Edit in the body" rationale — amended in place; the 2 stale doc-comments swept in-slice. **269 tests** (wide strip present/taps/archived-absent, `contact_detail_screen_test.dart` incl. busy-guard, direct `pane_header_test.dart`, a no-strip guard; `comments_section_test` Edit finders scoped to TextButton since EventDetailScreen now has an AppBar 'Edit' chip), analyze clean. Plan-critic **probe-verified** the strip layout (`Column`+`Expanded` in `Align`, no unbounded-height throw) and the GlobalKey wiring. **Process note:** a first cut shipped a bare pencil; corrected mid-slice to the labelled chip after user pushback — affordance choices belong in the plan against the app's established components (Decision 29's SubtleButton), not deferred to a QA "taste check". **OWED before merge: `/fullpush` gate (analyze ✓ · tests ✓ · build web · `/crlocal`) + light+dark emulator QA + push approval.**

---

**Prior: UI consistency pass — Decisions 47 + 48 — ✅ SHIPPED & MERGED (PR #55 → squash `154271b`; branch deleted; `main` clean & synced).** No migration → nothing to deploy; UI-only so installed apps keep working (**`/updatephone` owed only if you want the fixes on the S23+** — optional).
...
**Two follow-up fixes the review pipeline caught IN-BRANCH (the fleet earning its keep):**
- `53918b9` — post-commit **`semantic-reviewer`** caught the `chipTheme` fix's `ring: true` un-pinning the avatar disc (20→11px) so wide initials ("WW") spilled; my emulator QA passed only because the live data initials are narrow. Fixed via a new `InitialsAvatar.onChipFill` (colour, not a ring) + a `FittedBox` — **found by PROBING**, which also surfaced a `chip.dart` framework assertion that killed the `avatarBoxConstraints` route.
- `77bf0ae` — `/crlocal` (cloud-CR preview) caught `MetaLine` rendering `timestamptz` as raw UTC → wrong day near midnight east of UTC. Now `.toLocal()` first, matching `CommentsSection._timestamp`. Test uses a near-midnight-UTC input (adversarial, per the new QA rule).

**Decision 48 (`862ede0`) — the session's retro, two `learner`-promoted rules (approved):** reviewers **read + probe, never reason** (generalised the `agent-workflow.md` "CREATE OR REPLACE chain" rule from SQL-only to Flutter geometry/layout/hit-test/lazy-build); **visual QA asserts a SPECIFIED value on ADVERSARIAL data** (`docs/design-principles.md`). The ledger entry also records the meta-rule deliberately *NOT* written ("check adjacent axes"), per Decision 46's proxy-vs-intent logic — the strongest evidence for D46 is that learner used it to argue its own abstraction down.

**Cloud CR on PR #55 — ANSWERED (`8211be6`):** 2 minor findings, both triaged against source. FIX: picker doc-comment said `title` drives "AppBar copy only" but the empty state derives from it too (corrected to "all role copy"); the role/count test still asserted the retired `Attendees · N` (→ `People · N`). SKIP (recorded on the PR): CR wanted "No contacts yet / Add contacts first" → People, but that names the Contact **entity**, not the role — "Add people first" is nonsense; and the `title`→`noun` param rename is cosmetic churn. `/replycoderabbit` reply still to post (see §Owed).

**Verification across the branch:** analyze clean · **257 tests** (241 + 16, incl. mutation-proven gap-fills for the display-date path that had zero coverage) · web builds · on-device QA light+dark · `/fullpush` gate green (2 `/crlocal` rounds, converged).

**Deliberately deferred to separate slices (unchanged):**
- **Slice A2** — Edit → AppBar top-right (finding #2). Reverses Decision 29 (Edit in body so phone + AppBar-less desktop pane share one control); also a state refactor. Approved mockup exists.

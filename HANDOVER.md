> Cross-session work tracker. Update in place. Last updated: 2026-07-15 (Task importance — ✅ SHIPPED, ON BRANCH (feat/task-importance, `3bf48ea`); pending /fullpush/PR/merge/deploy — homebase deploy owed, installed clients need rebuild **post-deploy**. Decision 38. Prior: Pre-auth DB lockdown + Decision 37 MERGED & DEPLOYED (issue #3 CLOSED).).

# Handover

**Status: TASK IMPORTANCE — 0..3 priority marker (! / !! / !!!) — ✅ SHIPPED, ON BRANCH (feat/task-importance, commit `3bf48ea`; Decision 38; pending /fullpush/PR/merge/deploy).** New `importance smallint` column (0 = none, 1/2/3 = !/!!/!!!) + `p_importance` on `create_task(p_title, p_notes, p_contacts, p_importance)` and `update_task(p_id, p_title, p_is_done, p_notes, p_contacts, p_importance)` (drop+recreate — re-issues Decision 36 PUBLIC-execute revoke + anon/authenticated grant on the new signatures). Model/repo/form/detail/list wired; shared `ImportanceMarks` widget on rows + detail. Segmented None/!/!!/!!! picker on form. **Fixed semantic scale (chrome)** — amber/orange/red hues tuned per-theme for legibility (lib/util/importance.dart). Active tasks sort highest-importance first, newest. Fresh-DB verified: migrations apply, the new signatures exist, public execute revoked, round-trip + CHECK constraint hold. **185 tests green, analyze clean.** **OWED: homebase deploy** — `20260715130000_add_importance_to_tasks.sql` changes `update_task` signature (required `p_importance` parameter with no default), so installed clients error until rebuilt. Deploy, then `/updatephone` + `/updatelinux` (both hit the homebase schema for the new RPC arity).

> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (Desktop sidebar — Decision 28, Slice A — committed on `feat/desktop-sidebar` (unpushed); Slice B next).

# Handover

**Status: DESKTOP SIDEBAR — Decision 28, Slice A — committed on `feat/desktop-sidebar` (unpushed, commit `46795044`).**
Wide screens (≥600dp) now render a labelled `_Sidebar` (CRM+ mark, WORKSPACE nav, Settings pinned) instead of the compact `NavigationRail`; phone `NavigationBar` (<600dp) unchanged. Look stays Decision 13 mono theme (selection styling mirrors `navigationRailTheme` tokens: primaryContainer chip, onSurface w600, onSurfaceVariant). Chrome only — no data yet. C⁺ glyph opts out of textScaler; labels use Flexible+ellipsis for textScaler safety. Added width-parameterized test `test/home_shell_test.dart` (setSurfaceSize at ≥600dp and <600dp). Slice B (Contacts master-detail panel on wide screens, reusing existing detail/edit) queued next; approved mockup artifact drove the slice. Plan: `~/.local/share/claude-config/claude/plans/fuzzy-munching-thacker.md`.
- Gate: `flutter analyze` · **103 tests** · web build (Slice A adds tests, tests count up from 101 Tasks v0 → 103 after sidebar tests).
- **RESUME = plan + build Slice B (Contacts master-detail);** then: Tasks v0 phone QA owed (Decision 27, merged & deployed but not QA'd due to phone off-tailnet) · in-app empty-state hints (issue #21) · auth (GoTrue) + DB hardening (issue #3).

> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (RPC-for-all-writes: Slice 1 built on branch).

# Handover

**Status: RPC-FOR-ALL-WRITES (Decision 26) — IN FLIGHT. Slice 0 (auto-reload) DEPLOYED & VERIFIED; Slice 1 (contacts writes → RPC) BUILT ON BRANCH; next = Slice 2 (event_types writes → RPC).**
...
- ✅ **Slice 0 — DDL-watch triggers DEPLOYED & VERIFIED** (`670787c`, PR #25 + follow-up `chore/remove-manual-pgrst-reload` pending merge): `pgrst_watch()` + `pgrst_ddl_watch`/`pgrst_drop_watch` auto-reload PostgREST on any DDL. Deployed to homebase; verified live (create+drop test objects callable via `/rpc/` with no manual NOTIFY); triggers own running/steady-state + ad-hoc case; deploy script reframed to single unconditional `notify pgrst` as cold-start safety net (Decision 25 amended 2026-07-12). Gate was fully green (fresh-DB migration validation, implementation-critic, red-team 0-blocking, db-security-reviewer CLEAN, `/crlocal` 3/3, cloud CR pass). `main` clean & synced.
- ✅ **Slice 1 — contacts writes → RPC (BUILT ON BRANCH)** (`1988e26`, `feat/contacts-write-rpcs`): migration `20260712130000_contact_write_rpcs.sql` adds `create_contact` / `update_contact` SECURITY DEFINER RPCs (server-side trim + nullif normalization, update guards deleted_at, grants to anon+authenticated); `Contact.toRpcParams()` replaces `toWrite()`; `contacts_repository` routed to RPCs + refetch; `docs/database.md` rule #2 re-reversed (Decision 26 dated); `event_types_repository.dart` doc-comment patched. Tests green (69, all 12 migrations on fresh DB, RPC-as-anon verified). **Pending:** `/fullpush` → PR → merge → homebase deploy → phone QA.
- ⏭️ **NEXT — Slice 2:** event_types writes → RPC (`create_event_type` / `update_event_type`). Match the template: security definer, `set search_path = public`, grant anon+authenticated, `p_`-params, server-side `nullif(trim())`; new models need `toRpcParams()`. Then **Slice 3:** event_comments (if soft-delete RPC needed; currently rule #4 exception — check). `auth.uid()` deferred to #3.

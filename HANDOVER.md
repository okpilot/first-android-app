> Cross-session work tracker. Update in place. Last updated: 2026-07-14 (Task comments Slice 2a — CommentsSection extract refactor, branch-pending; `main`/notes merged in).

# Handover

**Status: TASK COMMENTS SLICE 2a — CommentsSection extract refactor — BUILT ON BRANCH `feat/task-comments` (commit `2717da9`; `main` merged in), pending /fullpush/PR/merge/deploy.**
A behavior-preserving refactor that extracts the private `_CommentsSection` widget from event detail into a generalized public `CommentsSection` widget, ready to serve task comments (Slice 2b). `comment.dart` field `eventId` → `parentId`, `toRpcParams()` removed. `SupabaseCommentsRepository` → `SupabaseEventCommentsRepository` (interface method `fetchForEvent` → `fetchFor`). Reads alias the FK column to `parent_id` so one model + shared widget work for any `*_comments` table. All async invariants survive (stale-guard, re-entrancy, mounted-after-await, _lastData). Full post-commit fleet clean (N=2 floor); test-writer added 2 standalone parent-agnostic tests.

**RESUME = Slice 2b (task_comments table + wire CommentsSection onto task detail)**, then continue with backlog candidates: #21 (in-app empty-state hints) · #3 (auth/GoTrue + DB hardening).

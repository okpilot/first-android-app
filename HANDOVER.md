> Cross-session work tracker. Update in place. Last updated: 2026-07-15 (Pre-auth DB lockdown, Decision 36 / issue #3 subset — 🔨 BUILT + fresh-DB-verified on branch `feat/preauth-db-lockdown`, NOT yet pushed).

# Handover

**Status: PRE-AUTH DB LOCKDOWN — issue #3 auth-independent subset (Decision 36) — 🔨 BUILT + VERIFIED on branch `feat/preauth-db-lockdown`, NOT yet pushed/merged/deployed.** One migration `20260715120000_preauth_lockdown.sql`, three behavior-preserving hardenings: (1) **closed the direct anon write path** on the 5 still-open mutable tables (revoked `insert,update` grants + dropped the `<table>_insert/update` RLS policies → the SECURITY DEFINER RPCs are the SOLE write path, matching events/event_attendees which were SELECT-only from day one); (2) **`revoke execute … from public`** on all **21** client-facing RPCs (the explicit anon/authenticated grants persist, so installed clients keep working); (3) added the Slice-2b-deferred **archived-task guard** to the 4 `task_comment` RPCs (require parent `tasks.deleted_at is null`). Safe because the client already writes exclusively via `.rpc()` (verified across `lib/data/*`) and reads direct. **Fresh-`postgres:15` verification PASSED** (all 5 boundary checks — see plan.md status bullet). **Rule-reversal-sync shipped in-slice:** decisions.md (D15/23/33 amended in place + **Decision 36** appended); 5 sibling migration headers annotated "superseded by 20260715120000" (comment-only, SQL untouched); database.md #2/#4; .coderabbit.yaml SQL instruction; db-security-reviewer + red-team phase-flip; backend/README.md verify curls. **RESUME =** run the post-commit fleet (+ db-security-reviewer) → `/fullpush` gate → PR + cloud CR → deploy to homebase (grants-only tighten, no signature change, so installed clients don't break — pair with `/updatephone`+`/updatelinux` only for good measure). **Remaining #3 after this = `auth.uid()` owner-RLS (needs GoTrue) + the `search_path=''` convention slice.**

> Cross-session work tracker. Update in place. Last updated: 2026-07-17 (UI consistency pass, Decision 47 — **COMMITTED ON BRANCH `feat/ui-consistency-a1`**; `/fullpush` + push approval pending. Prior: Soft-delete non-destructiveness curls, issue #19 / Decisions 45 + 46 — ✅ MERGED, PR #53 → squash `f582aba`; issue #19 CLOSED).

# Handover

**Status: UI consistency pass — Decision 47 — ✅ COMMITTED ON BRANCH `feat/ui-consistency-a1`; push/PR pending.** Slice A1 (findings #1/#3/#4/#6, 4 High-priority issues from the full UI/UX scan): one date format (`ymd()` doc-commented WIRE-ONLY, new `displayDate`/`displayDateNoYear`/`longDate` in `util/calendar.dart`), one Save per form (AppBar actions removed from contact/event/event_type/task_category editors), "People" one word everywhere (UI copy only; domain names like `Event.attendees`/`event_attendees` kept), one chip (`chipTheme` added). **241 tests** (236 + 5 new formatter tests for display dates), analyze clean, on-device QA light+dark. **Plan-critic ran 3 rounds / 3 revisions, 16 defects found and fixed** (two commit-blocking analyzer traps — orphaned imports + orphaned `_dateLabel`; a test target never built at default viewport; visible `chipTheme`/avatar fill collision fixed with `ring: true`). **Owed:** `/fullpush` + push approval; then slice A2. **No migration, no model/DB change** → `db-security-reviewer` / `red-team` don't trigger. Decision 47.

**Deliberately deferred to separate slices:**
- **Slice A2** — Edit → AppBar top-right (finding #2). Reverses Decision 29 (whose rationale: Edit in body so phone + AppBar-less desktop pane share one control).
- **Finding #5** (Delete → Archive). Rename honest only if contacts + events get an archived section + restore, like tasks already have.
- **Candidate rule** (from D47 Principle) — codify "critics must read source + probe, not reason" in `.claude/rules/agent-workflow.md` (its own change, touches `.claude/rules/*`, defer to a rules slice).
- **The other 16 scan findings** (Groups B/C/D) — phone search, comment composer, taxonomy drifts, SafeArea, 3-day truncation.

**RESUME:** `/fullpush` this branch. Prior slice status (issue #19): **The gap, precisely:** on the 4 tables whose SELECT is `using (deleted_at is null)` (`contacts`, `events`, `event_types`, `task_categories`), `soft_delete_*` returns `void` and the policy hides the row — so **nothing anon can observe distinguishes a soft-delete from a hard delete**. `task_categories` already had the superuser proof (README's `## Verify: task category write RPCs` block); this generalizes it to `contacts` + `event_types`, adds a **new `## Verify: event write RPCs + the attendee parent-gate (Decision 18)` section** (`events` had none), and pins two previously-unproven behaviours: a soft-deleted event type → embed reads `null` **while `events.type_id` still holds the id** (the FK is never cut — anon can see this itself, no privileged read needed), and `event_attendees` as the **only** child table hidden by a parent's soft-delete. **Decision 45:** privileged reads are DB-side `psql` as `postgres`, never a minted `service_role` key (none exists; it'd be a permanent bypass credential contradicting the D36 lockdown). Every check was **run end-to-end and produces the documented output verbatim**. Also fixed a stale D37 surface in the same file (`## To homebase (later)` still said "add GoTrue when the auth slice needs it").
**⚠️ Found while verifying — the LOCAL dev DB had drifted from `backend/migrations/`.** Volume inited 2026-07-11; `init.sh` applies migrations *only* on a fresh volume, and some were later hand-applied — so `create_contact`/`update_contact`, the event-type + comment write RPCs and `task_category_links` were all **missing**, and `create_event` was still the pre-D41 9-arg signature. Any local curl-run since 2026-07-12 was hitting a stale schema. Fixed via the documented `docker compose down -v && docker compose up -d` re-init (full chain applies clean, no errors). **Homebase was never affected** (D41 prod-verified there). Worth remembering: the local stack has no migration ledger, so it silently rots between re-inits.
**Decision 46 — the docs-sync rule was widened (from `learner`, count 3).** Worth calling out because the rule failed *through* itself: on this docs-only slice **every** blocking finding from `implementation-critic` (both rounds) was a stale doc twin, never the artifact under review. Two verified defects: (1) it triggers on reversing a **convention**, so a **status flip** (`/updatephone` owed → done) doesn't read as in-scope and the sweep never runs; (2) its subsection list "(Context / Implementation / Principle)" named an `Implementation` heading that **has never existed in the ledger**, while omitting `Decided`. Now two triggers (convention **or** status flip) and an explicitly **open** subsection list. **The rule's own wording then failed twice more, live:** draft 1 named only `Deploy note` → `semantic-reviewer` found a twin in a bare `Deploy` (D33); draft 2 said twins hide "wherever shipping is recorded" → it found one in D43's `Principle`, which records no shipping at all. So the operative instruction is now **grep the entry for the old status word** (and check the title against the body) — every draft that named a *place* was wrong. Same failure shape `semantic-reviewer` caught in D45 itself this slice: a rule keyed on a **proxy** (has a `deleted_at` column) instead of its intent (anon-invisibility) silently exempts every case the proxy misses.
**Review bar for this PR:** `db-security-reviewer` / `red-team` don't trigger (no `backend/migrations/**` change), and **both CR tiers skip this slice entirely** — D44's `path_filters` exclude `**/*.md` + `.claude/**`, and all 14 changed files are one or the other. The **in-house fleet was the whole review bar** (5 rounds of code-reviewer + semantic-reviewer; rounds 1-4 each found real issues, round 5 converged). CR silence on the PR is *expected*, not approval.
**✅ D44 watch-item (2) CLOSED — and the premise was wrong.** It said "no local validator for the globs; spot-check the first PR touching a root `.md`". There **is** a local validator: `/crlocal` runs `coderabbit review -c .coderabbit.yaml`, so it inherits the exact same `path_filters` as the cloud bot. Run here (2026-07-17, 14 `.md`/`.claude/**` files): **`Review failed: No files to review`, exit 1** — direct proof the globs match and exclude as intended, without waiting on a PR. Note the exit code: **1 means "nothing in scope", not a crash** — don't read it as a broken run. Watch-item (1) (doc-updater self-applying conservative lifecycle wording) also **CLOSED** — it did so unprompted this session, 0 findings.
**RESUME =** `/fullpush` this branch, then pick the next feature slice — **filter/group the task list BY category** (natural follow-on to Slice B), or **#46** (`_SwatchGrid` extraction + `ChipSection` atom + `TypeSwatch`→`lib/widgets/`), or **#21** empty-state hints.

---

**Status: ISSUE #10 CLOSED + review-pipeline retuned — all merged, `main` clean & synced.** This session shipped three slices: **D42** shared test fakes (`test/support/fakes.dart`, PR #50), **D43** shared `DetailField` widget (PR #51 → `2d450ac`) which **closed issue #10**, and **D44** cloud-CR scoping + doc-updater lifecycle hardening (PR #52 → `99358e0`). D44 scoped cloud CodeRabbit (`.coderabbit.yaml` `path_filters` exclude `**/*.md` + `.claude/**`) so the in-house fleet owns doc prose — **validated: #52's cloud review posted 0 findings** — and added a *Lifecycle-state discipline* rule to `doc-updater` (derive lifecycle words from real git/gh state; default "committed on branch").
**NEXT-SESSION WATCH (D44 follow-through):** (1) the `doc-updater` def change bites only **next session** (defs snapshot at start) — confirm it self-applies "committed on branch" wording **unprompted** on the first branch-only slice; still over-claims → the def text isn't landing, escalate. (2) Spot-check `path_filters` on the first PR touching a **root `.md`** — no local validator for the globs; if cloud CR still reviews it, add an explicit `!*.md`. Both are tracked in `learner`'s memory.
**RESUME (superseded — see the current Status block above):** pick the next feature slice — **filter/group the task list BY category**, or **#46**, or **#21** empty-state hints. `/updatephone` is **done** (2026-07-17, S23+ at HEAD `2c7a495` — clears the D41/40/39 debt); an on-device create round-trip stays in the Phone QA backlog. Optional emulator/desktop QA before the next UI slice.

---

**Prior: TASK CATEGORIES — task↔category join, many-per-task (Slice B) — ✅ SHIPPED, MERGED (PR #47 → squash `37a3e37`; branch deleted; `main` clean & synced) & DEPLOYED to homebase (migration `20260715150000_link_tasks_to_categories.sql` live; SSH-verified: new `create_task(…,uuid[])` 5-arg / `update_task(…,uuid[])` 7-arg sigs exist, PUBLIC execute revoked / anon granted, `task_category_links` RLS-on with anon SELECT-only / INSERT-denied). `/updatelinux` done (release bundle rebuilt vs homebase, URL baked no-localhost, HEAD `37a3e37`); `/updatephone` done 2026-07-17 (S23+ `R5CW71HWXKK` at HEAD `2c7a495` — covers Slice A's install too). Decision 40.** New `task_category_links` join table (composite PK, both FKs cascade, reverse index, RLS `using(true)` SELECT, no write policy/grant). `create_task(text, text, uuid[], smallint, uuid[])` and `update_task(uuid, text, boolean, text, uuid[], smallint, uuid[])` recreated via drop+recreate (old sigs dropped to avoid PGRST203; new sigs carry the lockdown revoke+grant re-issue; identical behaviour otherwise). `Task.categories` field with full TaskCategory embed + `copyWith` preservation; `toRpcParams` sends `p_categories`; all 6 reconstructing fakes updated (create/update/archive/restore/fetch-A/fetch-B) + exact `toRpcParams` map assertion checks categories. New `CategoryPickerScreen` (multi-select chips); `_CategoriesSection` on task form; dot+name category chips on row + `_CategoriesList` on detail (local `TypeDot`+`Text`, not the `EventType`-typed `TypeLabel`); 17 new tests (embed/toRpcParams/copyWith/picker/form/row/detail). **222 tests green, analyze clean, web builds.** Migration validated on fresh Postgres + on homebase. Full in-house fleet + db-security-reviewer CLEAN; `/crlocal` (2 rounds); cloud CR on #47 (6 findings → 5 FIX `6e1a230` + 1 SKIP; bot confirmed all in-thread).
...
- **Slice 1 (#13):** `event_types` table (RLS, `#RRGGBB` CHECK, soft-delete) + nullable
  ...
  is soft-deleted → the type is hidden, not erased. gate-green · 38 tests. *(Refined 2026-07-17,
  Decision 45 — the null embed **alone** proves only invisibility; the actual proof is the **pair**,
  a null embed beside a retained `type_id`, and only under the NO ACTION FK.)*

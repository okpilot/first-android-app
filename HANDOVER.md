> Cross-session work tracker. Update in place. Last updated: 2026-07-14 (Desktop-adaptive UI + Tasks desktop master-detail — Decision 28 Slices A–D — both PRs squash-merged to `main`; `main` clean & synced).

# Handover

**Status: DESKTOP-ADAPTIVE (Slices A–C) + TASKS DESKTOP (Slice D) — Decision 28 — ✅ SHIPPED & MERGED. `main` clean & synced.**
Both stacked PRs are in: **PR #31** (desktop-adaptive: sidebar + Contacts master-detail + desktop top/search + `CRM+` window title) squash-merged → `5a41c5b`; **PR #32** (Tasks desktop master-detail) squash-merged → `27ba471`. Both branches deleted (local + remote).
Tasks Slice D: on a wide content area (≥ `kTasksWideBreakpoint` = 640) Tasks is a **two-pane master-detail** — list pane (header "Tasks" + active count + inline "New") + `VerticalDivider` + an **in-place `TaskEditView`** on the right that swaps on selection (auto-selects first active; "New" opens a blank draft; edit/archive/restore reload + keep the row selected). **Narrow / phone unchanged** (AppBar + FAB + push `TaskFormScreen`). Extracted a Scaffold-less **`TaskEditView`** from `TaskFormScreen`; keyed by **`id:isArchived:isDone`** so archive/restore AND a list-toggled completion remount the control set; `_saving` reset on success; `_onEditorChanged` updates `_lastData` optimistically. Editor pane matches `ContactDetailView` (capped 560 + left-aligned, in-pane heading, compact toggle). Design journey (both live-QA rejections recorded): Direction A (centred list) built + rejected → Direction B; first B pane rejected as "ugly" → redesigned via the UI Build Checklist. Amends Decision 27's "no detail screen" for **wide only**. Suite **119**. Memory: `feedback-wide-layout-fill-space`.
**Cloud CodeRabbit fully answered on both PRs.** #31: 3 findings, all already-fixed (`6880d81`), recorded. #32: 2 review rounds → 5 findings, **all FIX** (`6577ae0` ×4 + `7844b3a` doc-count) — optimistic `_lastData`, empty-wide-draft fall-through, `isDone` remount, memory prose, stale test-coverage line; 2 regression tests added. CR status check `pass` before merge.
**RESUME = the owed backlog:** 📱 **Tasks v0 phone QA + RPC write-path QA on the S23+** (deferred while the device was off-tailnet) · then **#21** (in-app empty-state hints) or **#3** (auth/GoTrue + DB hardening). Or build the contact **activity-view slice** to fill the master-detail right-pane whitespace.

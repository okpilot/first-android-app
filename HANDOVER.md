...
**RESUME = pick the next thin slice.** Event-types is done & deployed; the `/coderabbit` split is
merged & dogfooded. No code blocker, no open PR. Strongest next candidates: **auth (GoTrue)** logins
+ owner-based RLS (unblocks the DB-hardening issue #3), or search/filter on the Contacts list.

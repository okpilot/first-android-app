> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (RPC-for-all-writes: Slice 0 merged).

# Handover

**Status: RPC-FOR-ALL-WRITES (Decision 26) — IN FLIGHT. Slice 0 (auto-reload) MERGED; homebase deploy + verify still owed.**
...
- ✅ **Slice 0 — DDL-watch triggers MERGED** (`670787c`, PR #25): `pgrst_watch()` + `pgrst_ddl_watch`/`pgrst_drop_watch` auto-reload PostgREST on any DDL. Gate fully green (fresh-DB migration validation, implementation-critic, red-team 0-blocking, db-security-reviewer CLEAN, `/crlocal` 3/3, cloud CR pass). `main` clean & synced.
- ⏭️ **RESUME HERE (Slice 0 not fully done):** (1) `deploy-homebase.sh` to apply the triggers; (2) **verify auto-reload live** — create+drop a throwaway object on homebase, confirm PostgREST serves a freshly-created object with NO manual `NOTIFY`; (3) **follow-up PR** removing the manual reload block from `deploy-homebase.sh` (~L57–65) + point the Decision 25 note at the triggers.
- ⏭️ **Then Slices 1–3** (contacts / event_types / comments writes → RPC). Slice 1 also **rewrites database.md rule #2** (Decision 26 knowingly re-reverses the "NOT everything is RPC" rule) + stale repo doc-comments; Slice 3 rewrites the rule #4 comments exception. Convention: match the event RPC template (`security definer`, `set search_path = public`, grant anon+authenticated, `p_`-params, server-side `nullif(trim())`); new models need `toRpcParams()`; `update_comment` is `(p_id,p_body)` only. events already fully RPC (template). `auth.uid()` deferred to #3.

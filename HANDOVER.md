> Cross-session work tracker. Update in place. Last updated: 2026-07-12 (TASKS v0 (Decision 27) built on branch `feat/tasks-v0` — analyze + 94 tests green, local curl verify passed; push gate + PR + deploy + phone QA still owed).

# Handover

**Status: TASKS (v0) — Decision 27 — 🔨 BUILT on branch `feat/tasks-v0`; push gate not yet run.**
A lightweight to-do list: add (form + FAB) · complete (tap the circle) · edit · archive · restore · view Completed + Archived (collapsible sections). New `tasks` table (`id`, `title`, `is_done`, `created_at`, `updated_at`, `deleted_at`), migration `20260712160000_create_tasks.sql`; **viewable soft-delete** reusing Decision 23's `using (true)` SELECT policy (2nd such table alongside `event_comments`); **writes via RPC** (Decision 26) — `create_task` / `update_task` (title+done in one path) / `soft_delete_task` / `restore_task`, SECURITY DEFINER for uniformity. Client: `Task` model + `TasksRepository` + `TasksListScreen` + `TaskFormScreen` + a 4th nav destination (Contacts · Calendar · **Tasks** · Settings), wired through `main`/`app`/`home_shell`. Plan-critic REVISE folded in (create_task arity trap → `toRpcParams()` is create-shape only; the 2nd `ContactsApp` caller in `calendar_screen_test.dart` fixed). **`flutter analyze` clean · 94 tests green (73→94) · local curl verify passed** (create/update/archive-still-selectable/P0002-guard/restore/blank-400/anon-DELETE-401). Full plan: `~/.local/share/claude-config/claude/plans/smooth-splashing-avalanche.md`.
**RESUME = run the post-commit fleet → `/fullpush` gate (analyze · test · build web · `/crlocal`) → push → PR → cloud-CR cycle → merge → `deploy-homebase.sh` (ledger 14 → 15) → phone QA via `/updatephone`.** The Slice 1–3 RPC phone QA (below) is still owed too — batch it with the Tasks phone QA.
